...
namespace Flowtime {
    public class Day : GLib.Object {
        ...
        private uint _worktime = 0;
        internal uint worktime {
            get {
                return _worktime;
            }
            set {
        ...
        private uint _breaktime = 0;
        internal uint breaktime {
            get {
                return _breaktime;
            }
            set {
                ...
                if (i->type == ELEMENT_NODE) {
                    switch (i->name) {
                        case "worktime":
                            ...
                            _worktime = uint.parse(worktime_node->get_content ());
                            break;

                        case "breaktime":
                            breaktime_node = XmlUtils.get_content_node (i, "breaktime");
                            _breaktime = uint.parse (breaktime_node->get_content ());
                            break;
                    }
                }
            }
